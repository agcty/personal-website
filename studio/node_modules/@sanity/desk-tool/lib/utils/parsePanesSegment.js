...
function encodeChunks(pane, index, group) {
  var payload = pane.payload,
      ...
      id = pane.id;
  var sameAsFirst = index !== 0 && id === group[0].id;
  ...
  var encodedParams = Object.keys(params).reduce((pairs, key) => {
    ...
    if (sameAsFirst && index !== 0 && !_paneRouter.exclusiveParams.includes(key) && ((_group$0$params = group[0].params) === null || _group$0$params === void 0 ? void 0 : _group$0$params[key]) === params[key]) {
      return pairs;
    }
...
function parsePanesSegment(str) {
  ...
  }).map((pane, i, siblings) => pane.id ? pane : _objectSpread(_objectSpread({}, pane), {}, {
    id: siblings[0].id
